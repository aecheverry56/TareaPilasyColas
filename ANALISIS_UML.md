...
classDiagram
    ...
    class Colas1~T~ {
        -NodoColaEnlazada~T~ primero
        -NodoColaEnlazada~T~ ultimo
        -int tamano
        +agregar(T) void
        +eliminar() T
        +alReves() Colas1~T~
    ...
    class NodoColaEnlazada~E~ {
        E dato
        NodoColaEnlazada~E~ siguiente
    }
    class Colas4~T~ {
        -NodoColaCircular~T~ primeroOcupado
        -NodoColaCircular~T~ primeroDisponible
        -int tamano
        +agregar(T) void
        +tomar() T
        +eliminar() T
    ...
    class NodoColaCircular~E~ {
        E dato
        NodoColaCircular~E~ siguiente
    }
    class Colas6~T~ {
        -ArrayDeque~T~ datos
        +agregar(T) void
    ...
    class Colas7 {
        -ArrayDeque~ClienteCine~ asiduos
        -ArrayDeque~ClienteCine~ ocasionales
        +agregar(String, boolean) void
        +tomar() ClienteCine
        +eliminar() ClienteCine
    }
    class ClienteCine {
        String nombre
        boolean asiduo
    ...
    class Colas8 {
        +asignar(ArrayDeque~ConsolaInventario~, ArrayDeque~SolicitudTienda~) List~AsignacionConsolas~
        +main(String[] args) void
    }
    ...
    class SolicitudTienda {
        String tienda
        int cantidad
    }
    class AsignacionConsolas {
        String tienda
        List~String~ codigos
    }
    class Colas9 {
        +asignar(ArrayDeque~MesaRestaurante~, ArrayDeque~ReservacionMesa~, boolean) List~AsignacionMesa~
        +main(String[] args) void
    }
    ...
    Main ..> Colas8
    Main ..> Colas9
    Colas1 *-- NodoColaEnlazada
    Colas4 *-- NodoColaCircular
    Colas7 *-- ClienteCine
    Colas8 ..> ConsolaInventario
    Colas8 ..> SolicitudTienda
    Colas8 ..> AsignacionConsolas
    Colas9 ..> MesaRestaurante
    Colas9 ..> ReservacionMesa
